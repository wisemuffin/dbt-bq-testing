...
WITH source as (
  ...
  phones as (SELECT companies.contactid, phones.value.phonetype, phones.value.phonenumber, phones.value.phoneareacode, phones.value.phonecountrycode
    FROM source companies,
 unnest(phones) as phones
 ),
      addresses as (SELECT companies.contactid, addresses.value.addresstype, addresses.value.addressline1, addresses.value.addressline2, addresses.value.addressline3, addresses.value.addressline4, addresses.value.city, addresses.value.region, addresses.value.country, addresses.value.postalcode
         FROM source companies,
