...
renamed as (
  SELECT
      ...
      lineitems.value.description as transaction_description,
      split(lineitems.value.accountcode,' ')[SAFE_OFFSET(0)] as account_code,
      currencycode as transaction_currency,
      cast(null as numeric) as transaction_exchange_rate,
      lineitems.value.lineamount as transaction_gross_amount,
      cast(null as numeric) as transaction_fee_amount,
      lineitems.value.taxamount as transaction_tax_amount,
      lineitems.value.lineamount - lineitems.value.taxamount as transaction_net_amount,
